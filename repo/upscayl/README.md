# Mero local Upscayl usage

This folder is a vendored copy of [Upscayl](https://github.com/upscayl/upscayl) inside the `mero-depth` repo.

Upscayl provides:

- **A desktop UI**: Electron app. Run from source with `npm install` then `npm run dev`.
- **A CLI binary**: `resources/linux/bin/upscayl-bin`, already included for Linux. This is easiest for batch work.
...
./mero-upscale.sh \
  "/path/to/input.png" \
  "/path/to/output-4x.png"
```

2x output using the lighter model:
...
./resources/linux/bin/upscayl-bin \
  -i "/path/to/input.png" \
...
Useful bundled models:

- `upscayl-standard-4x`: general default
- `upscayl-lite-4x`: faster/lighter
- `ultrasharp-4x`: sharper output
- `ultramix-balanced-4x`: balanced style
- `digital-art-4x`: art/anime-ish images
- `high-fidelity-4x`: detailed output
- `remacri-4x`: alternate high-detail model

## Desktop UI

Yes, Upscayl offers a desktop UI. This repo is an Electron app.
...
```bash
cd ~/projects/depthflow/repo/upscayl
./mero-upscale.sh to_upscale.jpeg /tmp/upscayl-wrapper-test.png 2 upscayl-lite-4x
```

Produced a `512x512` PNG from the bundled `256x256` sample using the NVIDIA GTX 1060.
